...
def get_deployment_path(skill_config):
    """Get the deployment target path from config"""
    target = skill_config.get('deployment', {}).get('target_path', '')
    if target:
        # Resolve relative to config file location
        config_dir = Path(__file__).parent
        target_path = config_dir / target
        return target_path.resolve()
    # Default to skill's app directory (old behavior)
    return Path("app")


def is_generated_module(namespace, module, deployment_root=None):
    """Check if a module was generated by this tool by looking for our marker"""
    if deployment_root is None:
        deployment_root = Path(".")

    module_decl_path = deployment_root / f"app/etc/modules/{namespace}_{module}.xml"
    if not module_decl_path.exists():
        return False

    try:
        content = module_decl_path.read_text()
        # Look for our generator marker
        return 'generator="mahocommerce-builder"' in content or '<!-- Generated by Maho Module Builder -->' in content
    except:
        return False


def run_post_deployment(skill_config, namespace, module):
    """Run post-deployment tasks like composer dump-autoload and cache flush"""
    deployment_config = skill_config.get('deployment', {})
    paths_config = skill_config.get('paths', {})
    deployment_root = get_deployment_path(skill_config)

    success = True

    # Run composer dump-autoload if configured
    if deployment_config.get('auto_composer', False):
        print("\n[POST-DEPLOY] Running composer dump-autoload...")
        composer_cmd = paths_config.get('composer', 'composer')
        allow_superuser = paths_config.get('composer_allow_superuser', False)

        cmd = []
        if allow_superuser:
            cmd.extend(['COMPOSER_ALLOW_SUPERUSER=1', composer_cmd])
        else:
            cmd.append(composer_cmd)
        cmd.extend(['dump-autoload'])

        try:
            result = subprocess.run(' '.join(cmd), shell=True, capture_output=True, text=True, cwd=str(deployment_root))
            if result.returncode == 0:
                print("  ✓ Composer autoload updated")
            else:
                print(f"  ⚠ Composer warning: {result.stderr}")
                success = False
        except Exception as e:
            print(f"  ⚠ Failed to run composer: {e}")
            success = False

    # Flush Maho cache if configured
    if deployment_config.get('auto_flush_cache', False):
        print("\n[POST-DEPLOY] Flushing Maho cache...")
        maho_cli = paths_config.get('maho_cli', './maho')

        try:
            result = subprocess.run([maho_cli, 'cache:flush'], capture_output=True, text=True, cwd=str(deployment_root))
            if result.returncode == 0:
                print("  ✓ Cache flushed successfully")
            else:
                print(f"  ⚠ Cache flush warning: {result.stderr}")
                success = False
        except Exception as e:
...
def generate_module(config, skill_config=None):
    ...
    if conflicts:
        ...
        if is_generated_module(namespace, module, deployment_root):
            print("\n⚠️  Module exists but was generated by this tool")
    ...
    # Check for entity name conflicts with core Mage modules
    print("[CHECK] Checking for core module conflicts...")
    entity_conflicts = check_entity_conflicts(entities)
    if entity_conflicts:
        warning = format_conflict_warning(entity_conflicts)
        print(warning)
        ...
        if is_interactive:
            ...
            print("⚠️  Warning: Entity conflicts detected but continuing in non-interactive mode")
    else:
        print("  ✓ No core module conflicts found")
    ...
    code_path = deployment_root / f"app/code/{code_pool}/{namespace}/{module}"
    ...
    adminhtml_design_path = deployment_root / "app/design/adminhtml/default/default"
    adminhtml_layout_path = adminhtml_design_path / "layout" / namespace.lower() / f"{module.lower()}.xml"

    frontend_design_path = deployment_root / "app/design/frontend/base/default"
...
"""
    ...
    # Get relationships from config or auto-detect (with proper relationship names)
    relationships = detect_many_to_many_relationships(config, entities)
    if relationships:
        print(f"\n[RELATIONSHIPS] Found {len(relationships)} many-to-many relationships:")
        for rel in relationships:
            print(f"  - {rel[0]} ↔ {rel[1]} (table: {rel[2]})")

    # Add relationship tables to config.xml
    if relationships:
        for rel in relationships:
            # rel is now a tuple: (entity1, entity2, rel_name)
            rel_name = rel[2] if len(rel) > 2 else f"{rel[0]}_{rel[1]}"
            model_entities += f"""                    <{rel_name}>
                        <table>{namespace.lower()}_{module.lower()}_{rel_name}</table>
                    </{rel_name}>
"""
...
"""
    ...
    for entity in entities:
        ...
        files_created.extend(generate_entity_files(
            namespace, module, entity, entities, code_path, year, relationships
        ))
...
def generate_entity_files(namespace, module, entity, all_entities, code_path, year, relationships=None):
    ...
    generate_admin_blocks(namespace, module, entity, all_entities, code_path, files_created, relationships or [])
...
def map_field_type_to_ddl(field_type):
    """Map JSON field type to Maho DDL type constant (with synonym support)"""
    # Normalize type first (handles tinyint->smallint, timestamp->datetime, etc.)
    ...
    return type_map.get(normalized_type, 'Maho\\Db\\Ddl\\Table::TYPE_TEXT')
...
def generate_column_definition(field):
    ...
    normalized_type = normalize_field_type(field_type)  # Support type synonyms
    ddl_type = map_field_type_to_ddl(field_type)

    # Determine length
    if normalized_type in ['varchar', 'text']:
        raw_length = field.get('length', 255 if normalized_type == 'varchar' else '64k')
    ...
    # Nullable - use is_field_required() which supports both 'required' and 'nullable'
    required = is_field_required(field)
    if normalized_type not in ['timestamp', 'datetime']:  # timestamps have special handling
        options.append(f"        'nullable'  => {'false' if required else 'true'},")

    # Default value (TEXT/BLOB columns cannot have defaults in MySQL)
    if 'default' in field and normalized_type not in ['text', 'textarea', 'wysiwyg']:
    ...
    if normalized_type in ['int', 'smallint'] and field.get('unsigned', False):
    ...
    # Comment
    from lib.field_utils import get_field_label
    comment = field.get('comment', get_field_label(field))
...
"""
    ...
    if relationships:
        for rel in relationships:
            # rel is a tuple: (entity1, entity2, rel_name)
            entity1 = rel[0]
            entity2 = rel[1]
            rel_table_name = rel[2] if len(rel) > 2 else f"{entity1}_{entity2}"
            tables += f"""
/**
...
if __name__ == '__main__':
    ...
    if skill_config.get('deployment', {}).get('target_path'):
        print(f"✓ Using deployment path: {get_deployment_path(skill_config)}")

    try:
        generate_module(config, skill_config)

        # Run post-deployment tasks if auto_deploy is enabled
        if skill_config.get('deployment', {}).get('auto_deploy', False):
            run_post_deployment(skill_config, config['namespace'], config['module'])
